...
public class UserImageStorageImpl implements ImageStorage {
    ...
    public UserImageStorageImpl(tn.teams.workshopuploadimage.configimage.FileStorageProperties fileStorageProperties) {
        this.imageLocation= Paths.get(fileStorageProperties.getUploadImgUsersDir()).toAbsolutePath().normalize();
        try {

        }catch (Exception e){
            throw new tn.teams.workshopuploadimage.exception.FileStorageException("could not create the directory where the uploaded images will be stored",e);
        }
    }
    ...
    public String store(MultipartFile file) {
        ...
        try {
           if(fileName.contains("..")){
                throw new tn.teams.workshopuploadimage.exception.FileStorageException("File name contains invalid path sequence "+fileName);
            }
